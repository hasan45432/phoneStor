...
export default function ProductsPage() {
  ...
  const { dataPost, responsePost, loadingPost, errorPost, fetchPost } =
    usePost();
  ...
  const addNewProduct = async (e) => {
    ...
    // قبل از ارسال داده‌ها، می‌توانید خطایابی یا اعتبارسنجی کنید

    await fetchPost("http://localhost:3000/api/products", formData);
     fetchDataServer();
  };
  ...
  const fetchDataServer = async () => {
    ...
    const datassss = useCombinedStore.getState().datas;
    console.log(datassss);
    setProducts(datassss);
  };
